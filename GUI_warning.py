...
class GridButtonFrame(ctk.CTkFrame):
    def __init__(
        self, master, row, cols, call_back_functions, buttons_per_row=2, **kwargs
    ):
        ...
        self.top_border.grid(
            row=0,
            column=0,
            columnspan=self.buttons_per_row,
            sticky="ew",
            pady=(1, 0),
            padx=3,
        )
    ...
    def add_buttons(
        self, cols_indecies: list[list], call_back_functions: dict[callable]
    ):
...
class ConflictFrame(InteractiveFrame):
    def __init__(self, master, row_span=2, row=1, **kwargs):
        super().__init__(
            master=master, label="Conflits Détectés", color=light_blue, **kwargs
        )
        self.grid(
            row=row,
            rowspan=row_span,
    ...
    def add_button_frames(self, problematic_rows: dict, functions_callback_dict: dict):
        for row, cols in problematic_rows.items():
            self.button_frames.append(
                GridButtonFrame(
                    self.scrollable_frame, row, cols, functions_callback_dict
                )
            )
...
class WarningFrame(InteractiveFrame):
    ...
    def __init__(self, master, row_span=2, row=1, **kwargs):
        super().__init__(
            master=master, label="Avertissements", color=light_orange, **kwargs
        )
